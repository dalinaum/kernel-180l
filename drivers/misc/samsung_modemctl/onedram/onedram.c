...
#if defined (CONFIG_KOR_MODEL_M180L) // temp
#define VERSION_MAGIC	            (0x93507124)
extern unsigned int HWREV;

static int put_hwrev(struct onedram *od)
{
   unsigned int tmp;
   u32 uMagic = VERSION_MAGIC;

   unsigned char *dpram_base= (unsigned char *)od->mmio;
   u32 *magic_addr = (u32 *)(dpram_base+8);
   u32 *hwrev_addr = (u32 *)(dpram_base+12);

   *magic_addr = VERSION_MAGIC;
   *hwrev_addr = HWREV;

   tmp = *hwrev_addr;
   dev_dbg(od->dev, "Onedram put_hwrev: %d\n", tmp);
