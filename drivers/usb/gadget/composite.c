...
//		INFO(cdev, "mute_switch is unset. composite_disconnect\n");
		...
		if(cdev->products && cdev->products->mode == 0x8 && skip_condition < 2) { // if USBSTATUS_VTP
			INFO(cdev, "1**********skip mute_switch %d\n", skip_condition);
			skip_condition++;
#ifdef CONFIG_USB_ANDROID_ACCESSORY
		} else if(cdev->products && cdev->products->mode == 0x100 && skip_condition < 2) { // if USBSTATUS_ACCESSORY
			INFO(cdev, "1**********skip mute_switch %d\n", skip_condition);
			skip_condition++;
#endif
		} else {
